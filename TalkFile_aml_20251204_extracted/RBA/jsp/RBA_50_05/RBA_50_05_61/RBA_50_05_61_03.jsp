...
<%
	...
	String GYLJ_G_C            = Util.nvl(request.getParameter("GYLJ_G_C"));
...
// 	    form1.POSITION_NAME.value = POSITION_NAME;
        	   ...
        	   ,export : {allowExportSelectedData: false,enabled: false,excelFilterEnabled: false}
	     	   ...
	     	   ,columns: [
	     	        {dataField: "HNDL_P_NM"		,caption: "결재자"		,alignment: "center",allowResizing: true,allowSearch: true,allowSorting: true,allowEditing: false},
	      			...
	      			{dataField: "POSITION_NAME"		,caption: "직위"	,alignment: "left"	,allowResizing: true,allowSearch: true,allowSorting: true,allowEditing: false},
	...
	function doApproval(FLAG) {
        ...
        showConfirm(mesg,mesg2,function(){
    		...
    		params.pageID 	  = "RBA_50_03_02_02";
    		params.BAS_YYMM   = BAS_YYMM ;//기준연도
    		params.TABLE_NM   = "SRBA_TJACT_IMPRV_I";
    		params.GYLJ_FLAG  = FLAG; // 결재FLAG (0:승인요청,1:반려,2:승인)
    		params.GYLJ_G_C   = GYLJ_G_C;  //결재구분코드
...
<div class="button-area" style="float:right">
    ...
    <% if ( "105".equals(ROLEID) ) { // 105:RBA 담당자 %>
    ...
    <% } %>
    <% if ( "6".equals(ROLEID) ) { // 6:RBA/AML책임자 %>
       	${btnel.getButton(outputAuth, '{btnID:"btn_07", cdID:"apprRequestBtn", defaultValue:"결재요청", mode:"U", function:"doApproval(0)", cssClass:"btn-36"}')}
    ...
    <% } %>
    <% if ( "4".equals(ROLEID) ) { // 4:컴플라이언스운영팀 담당자 %>
       	${btnel.getButton(outputAuth, '{btnID:"btn_07", cdID:"apprRequestBtn", defaultValue:"결재요청", mode:"U", function:"doApproval(0)", cssClass:"btn-36"}')}
       	${btnel.getButton(outputAuth, '{btnID:"btn_03", cdID:"okBtn", defaultValue:"결재", mode:"U", function:"doApproval(2)", cssClass:"btn-36"}')}
       	${btnel.getButton(outputAuth, '{btnID:"btn_04", cdID:"refuseBtn", defaultValue:"반려", mode:"U", function:"doApproval(1)", cssClass:"btn-36"}')}
    <% } %>
