...
<%
    ...
    try{
        output = RBA5007.getSearch10(input);
    ...
    <%
        if(output.getCount("RANK") == 0){
            ...
            for( int i = 0; i < output.getCount("RANK"); i++){
                ...
                if(i == output.getCount("BRNO_NM") - 1){
				    %>
				        {
				            "통제점수": "<c:out value='<%=tongjeEffRt %>'/>",
				            "위험점수"  : "<c:out value='<%=mltfRskScor %>'/>",
				            "tag"       :"<c:out value='<%=output.getText("BRNO_NM" , i) %>'/>"
				        },
				    <%
                }else{
				    %>
    ...
    function PopulatePatientSamples(chart1){
        ...
        <%
                for( int i = 0; i < output.getCount("RANK"); i++){
                    ...
                    if((i == output.getCount("BRNO_NM") - 1) == false){
        %>
            {
            	"통제점수": "<c:out value='<%=tongjeEffRt %>'/>",
                "위험점수"  : "<c:out value='<%=mltfRskScor %>'/>",
                "부서"      :"<c:out value='<%=output.getText("BRNO_NM" , i) %>'/>"
            }, {
        },
        <%
                    }else{
