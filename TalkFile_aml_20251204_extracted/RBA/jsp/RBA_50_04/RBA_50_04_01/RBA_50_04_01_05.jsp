	  		...
	  		<c:when test="${ fn:length( output.EvalDetails  ) > 0 }">
		  		<c:forEach items="${output.EvalDetails }"   var="evalDetails" >
				  	<tr>
						...
						<td style="border-right:1px solid #CCCCCC" align="center">
							<select id="AUTO_EXT_YN" name="AUTO_EXT_YN" style="width: 100%" class="dropdown" value="${evalDetails.AUTO_EXT_YN}"  >
								<option value='0' <c:if test = "${ evalDetails.AUTO_EXT_YN eq '0' }" >selected="selected"</c:if>>N</option>
								<option value='1' <c:if test = "${ evalDetails.AUTO_EXT_YN eq '1' }" >selected="selected"</c:if>>Y</option>
							</select>
						</td>
					</tr>
				</c:forEach>
