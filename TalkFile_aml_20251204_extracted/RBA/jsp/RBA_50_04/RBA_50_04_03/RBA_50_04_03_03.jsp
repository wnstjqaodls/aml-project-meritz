...
// 	var GridObj1 	= new GTActionRun();
    ...
    $(document).ready(function() {
        ...
        if( "${ROLE_IDS}" == "4"  || "${ROLE_IDS}" == "104"  || "${ROLE_IDS}" == "6" ) {
        	$("#btn_07").hide();
    ...
    /*	$("#GRID_CNT2").text(cnt);
    	...
    	for( i=0; i < cnt ; i++ ) {
    		...
    		//복수Y/N선택
    		if( cellData.CHECK_YN == "Y"){
    			count_y++;
				str = "<div class='content'><div class='radio-list'><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+"' name='" + cellData.SNO + "' value='Y' onclick='calcount()' checked><label for='CHECK_YN_"+i+"'>Yes</label></div><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+1000+"' name='"+cellData.SNO+"' value='N' onclick='calcount()' ><label for='CHECK_YN_"+i+1000+"'>No</label></div></div></div></td>";
			} else if( cellData.CHECK_YN == "N"){
				str = "<div class='content'><div class='radio-list'><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+"' name='" + cellData.SNO + "' value='Y' onclick='calcount()' ><label for='CHECK_YN_"+i+"'>Yes</label></div><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+1000+"' name='"+cellData.SNO+"' value='N' onclick='calcount()' checked><label for='CHECK_YN_"+i+1000+"'>No</label></div></div></div></td>";
			} else {
				str = "<div class='content'><div class='radio-list'><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+"' name='" + cellData.SNO + "' value='Y' onclick='calcount()' ><label for='CHECK_YN_"+i+"'>Yes</label></div><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+1000+"' name='"+cellData.SNO+"' value='N' onclick='calcount()' ><label for='CHECK_YN_"+i+1000+"'>No</label></div></div></div></td>";
			} 
   			/* str = "<div class='content'><div class='radio-list'><div class='radio-item'><input type='radio' name='CHECK_YN_"+i+"' id='CHECK_YN_"+i+"' value='Y' ><label for='CHECK_YN_"+i+"'>Yes</label></div><div class='radio-item'><input type='radio' name='CHECK_YN_"+i+"' id='CHECK_YN_"+i+"' value='N' checked><label for='CHECK_YN_"+i+"'>No</label></div></div></div></td>"; */

   			if( cellData.CNTL_ITEM_DE_C == 1 ) {
   				color++;
   			}
   			
   			if( color == 1 ) {
   				$('#jipyoTable > tbody:last').append('<tr>'
   		   			    + '<td style="width:0;color:white;background-color: #b4f1e842;">' + cellData.SNO + '</td>'
   		   			    + '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:5%;background-color: #b4f1e842;" id="CNTL_ITEM_DE_C_'+i+'">'+cellData.NO+'</td>'  //
   		   	       		+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;color:blue;cursor:pointer;background-color: #b4f1e842;" id="CNTL_ITEM_C_'+i+'" onclick="javascript:callSample('+ cellData.CNTL_ITEM_C +')" >'+cellData.CNTL_ITEM_C+'</td>'  //통제평가코드(고객번호)
   		   	       		+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;background-color: #b4f1e842;" id="CNTL_ITEM_C_'+i+'">'+cellData.CNTL_ITEM_DE_C+'</td>'
   		   	   			+ '<td style="text-align:left;border-right: 1px solid #CCCCCC;width:60%;background-color: #b4f1e842;" id="CNTL_ITEM_DE_CTNT_'+i+'">'+cellData.CNTL_ITEM_DE_CTNT+'</td>'			//통제평가세부내용
   		   	   			+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:20%;background-color: #b4f1e842;">'														//입력값
   		   	            + str
   		   	            + '</tr>');
   				...
   				$('#jipyoTable > tbody:last').append('<tr>'
   		   			    + '<td style="width:0;color:white;background-color: #ff8b6021;">' + cellData.SNO + '</td>'
   		   			    + '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:5%;background-color: #ff8b6021;" id="CNTL_ITEM_DE_C_'+i+'">'+cellData.NO+'</td>'  //
   		   	       		+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;color:blue;cursor:pointer;background-color: #ff8b6021;" id="CNTL_ITEM_C_'+i+'" onclick="javascript:callSample('+ cellData.CNTL_ITEM_C + ')" >'+cellData.CNTL_ITEM_C+'</td>'  //통제평가코드(고객번호)
   		   	       		+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;background-color: #ff8b6021;" id="CNTL_ITEM_C_'+i+'">'+cellData.CNTL_ITEM_DE_C+'</td>'
   		   	   			+ '<td style="text-align:left;border-right: 1px solid #CCCCCC;width:60%;background-color: #ff8b6021;" id="CNTL_ITEM_DE_CTNT_'+i+'">'+cellData.CNTL_ITEM_DE_CTNT+'</td>'			//통제평가세부내용
   		   	   			+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:20%;background-color: #ff8b6021;">'														//입력값
   		   	            + str
   		   	            + '</tr>');
   				...
   				$('#jipyoTable > tbody:last').append('<tr>'
   		   			    + '<td style="width:0;color:white;background-color: #f7ff6021;">' + cellData.SNO + '</td>'
   		   			    + '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:5%;background-color: #f7ff6021;" id="CNTL_ITEM_DE_C_'+i+'">'+cellData.NO+'</td>'  //
   		   	       		+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;color:blue;cursor:pointer;background-color: #f7ff6021;" id="CNTL_ITEM_C_'+i+'" onclick="javascript:callSample('+ cellData.CNTL_ITEM_C + ')" >'+cellData.CNTL_ITEM_C+'</td>'  //통제평가코드(고객번호)
   		   	       		+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;background-color: #f7ff6021;" id="CNTL_ITEM_C_'+i+'">'+cellData.CNTL_ITEM_DE_C+'</td>'
   		   	   			+ '<td style="text-align:left;border-right: 1px solid #CCCCCC;width:60%;background-color: #f7ff6021;" id="CNTL_ITEM_DE_CTNT_'+i+'">'+cellData.CNTL_ITEM_DE_CTNT+'</td>'			//통제평가세부내용
   		   	   			+ '<td style="text-align:center;border-right: 1px solid #CCCCCC;width:20%;background-color: #f7ff6021;">'														//입력값
   		   	            + str
   		   	            + '</tr>');
    	...
    	if(cnt > 0) {
    		...
    		var score = count_y * 100 / cnt ;
        	//alert( "cnt : " + cnt + " : y count : " + count_y + " score : " + score );
        	totalscore = score.toFixed(2);
			
        	if ( "${ROLE_IDS}" == '7' || "${ROLE_IDS}" == "104" || "${ROLE_IDS}" == '4' ) {
	    		//복수Y/N선택
	    		$('#jipyoTable > tbody:last').append( '<tr><td colspan="5" style="text-align:center;border-right: 1px solid #CCCCCC; background-color:#eaeaea">${msgel.getMsg("RBA_50_04_03_01_012","통제효과성")}</td>'
    ...
    function callSample( CNTL_ITEM_C ) {

    	//alert( "${BAS_YYMM}.substring(0,4) : " + "${BAS_YYMM}".substring(0,4) );
    	//return ;
    	
    	var form2  = document.form2;
    	form2.pageID.value = "RBA_50_04_03_08";
    	var win = window_popup_open(form2.pageID.value, 1200, 350, '','yes');
       	
    	form2.BAS_YYYY.value = "${BAS_YYMM}".substring(0,4);
        form2.VALT_TRN.value = Number("${BAS_YYMM}".substring(4,6));
        
        //alert( "BAS_YYMM : " + "${BAS_YYMM}" );
        
        form2.CNTL_ELMN_C.value = "${CNTL_ELMN_C}";
        form2.BRNO.value = form1.BRNO.value;
...
/* 		var tbl = document.getElementById('jipyoTable');
    ...
    function doSave() {
    	...
    	var tabObj = document.getElementById("jipyoTable");
	   	...
	   	//alert( "call : " + tabObj.rows[1].cells[0].innerText + " : " + tabObj.rows[2].cells[3].innerText + " : "+ tabObj.rows[3].cells[3].innerText + " : ");
	   	
	   	//alert( "maxRow : " + maxRow );

	   	for(var i=1; i < maxRow-1; i++) {

	   		SNO    = tabObj.rows[i].cells[0].innerText;

			if( $("input:radio[name="+SNO+"]:checked").val() === null || $("input:radio[name="+SNO+"]:checked").val() === undefined ) {
				//alert( "SNO : " + SNO   + " : i : " + i );
				showAlert('미입력 항목이 존재합니다. 수정 할수 없는 상태 입니다', 'WARN');
				return;
			}
	   	}
   ...
   function doSave_Action(){
	    var tabObj = document.getElementById("jipyoTable");
	   	...
	   	for(var i=1; i<maxRow-1; i++) {
	   		...
	   		SNO    = tabObj.rows[i].cells[0].innerText;
	   		CHECK_YN   = $("input:radio[name="+SNO+"]:checked").val(); //$("#"+CNTL_ITEM_DE_C).val();

	   		//alert( "call : " + i + " : " + CHECK_YN );

	   		dataStr += SNO+"#"+CHECK_YN;
	   		dataStr += ",";
	   		//break;
		...
		var obj = new Object();
		obj.pageID            = "RBA_50_04_03_03";					//"RBA_50_04_03_02";
...
<form name="form2" method="post" >
    ...
    <input type="hidden" name="BAS_YYYY"        id = "BAS_YYYY"     >
    <input type="hidden" name="VALT_TRN"        id = "VALT_TRN"     >
        ...
        <div class="panel-footer" >
             <div class="table-box" style="display:block;height:calc(35vh - 80px);overflow:auto;">
                <table class="grid-table-type" id="jipyoTable">
	                    <tr>
	                    	<thead>
	                        <th>${msgel.getMsg('RBA_50_04_01_02_017','▼')}</th>
	                        <th>${msgel.getMsg('RBA_50_04_01_02_014','No')}</th>
	                        <th>${msgel.getMsg('RBA_50_04_01_02_016','고객ID')}</th>
	                        <th>${msgel.getMsg('RBA_50_04_01_02_014','No')}</th>
