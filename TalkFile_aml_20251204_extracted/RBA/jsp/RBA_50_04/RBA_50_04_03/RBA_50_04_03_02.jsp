...
// 	var GridObj1 	= new GTActionRun();
    ...
    function doSearch_end(dataSource) {
    	...
    	for( i=0; i < cnt ; i++ ) {
    		...
    		// 평가유형 ( 단일 Y/N 선택, 복수 Y/N 선택, 실적(건수 등) 입력 ) 따른 입력항목 세팅   CNTL_ITEM_C
    		//단일Y/N선택
   			if( cellData.CHECK_YN == "Y"){
   				count_y++;
   				str = "<div class='content'><div class='radio-list'><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+"' name='" + cellData.CNTL_ITEM_DE_C + "' value='Y' onclick='calcount()' checked><label for='CHECK_YN_"+i+"'>Yes</label></div><div class='radio-item'><input type='radio' id='CHECK_YN_"+ i+1000 +"' name='"+cellData.CNTL_ITEM_DE_C+"' value='N' onclick='calcount()' ><label for='CHECK_YN_"+ i+1000 +"'>No</label></div></div></div></td>";
   			} else {
   				str = "<div class='content'><div class='radio-list'><div class='radio-item'><input type='radio' id='CHECK_YN_"+i+"' name='" + cellData.CNTL_ITEM_DE_C + "' value='Y' onclick='calcount()'><label for='CHECK_YN_"+i+"'>Yes</label></div><div class='radio-item'><input type='radio' id='CHECK_YN_"+ i+1000 +"' name='"+cellData.CNTL_ITEM_DE_C+"' value='N' onclick='calcount()' checked><label for='CHECK_YN_"+ i+1000 +"'>No</label></div></div></div></td>";
   			}
   			...
   			$('#jipyoTable > tbody:last').append('<tr><td style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;" id="CNTL_ITEM_DE_C_'+i+'">'+cellData.CNTL_ITEM_DE_C+'</td>'  //통제평가세부코드
   	   				...
   	   				+'<td style="text-align:left;border-right: 1px solid #CCCCCC;width:70%;" id="CNTL_ITEM_DE_CTNT_'+i+'">'+cellData.CNTL_ITEM_DE_CTNT+'</td>'			//통제평가세부내용
    	...
    	if(cnt > 0) {
        	...
        	if ( "${ROLE_IDS}" == '7' || "${ROLE_IDS}" == "104" || "${ROLE_IDS}" == '4' ) {
    		        ...
    		        $('#jipyoTable > tbody:last').append( '<tr><td colspan="2" style="text-align:center;border-right: 1px solid #CCCCCC; background-color:#eaeaea">${msgel.getMsg("RBA_50_04_03_01_012","통제효과성")}</td>'
        	        + '<td colspan="1" id="totalscore" style="text-align:center;border-right: 1px solid #CCCCCC;width:10%;">' + totalscore + ' %</td>'
                    + '</tr>');
        	}
   ...
   function doSave_Action(){
	    var tabObj = document.getElementById("jipyoTable");
	   	...
	   	for(var i=1; i<maxRow-1; i++) {
	   		...
	   		CHECK_YN   = $("input:radio[name="+CNTL_ITEM_DE_C+"]:checked").val(); //$("#"+CNTL_ITEM_DE_C).val();
