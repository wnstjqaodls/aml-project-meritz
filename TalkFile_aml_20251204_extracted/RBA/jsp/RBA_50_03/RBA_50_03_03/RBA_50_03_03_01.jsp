...
********************************************************************************************************************************************
* Project         : RBA
* File Name       : RBA_50_03_03_01.jsp
* Description     : 위험평가 배점관리
...
<script>
    ...
    $(document).ready(function(){
        ...
        //$("#RSK_CATG").attr("style","width:100px;text-align:center;");
    ...
    function doSearch() {
        ...
        params.pageID		= pageID;					//RBA_50_03_03_01
        params.classID		= classID;					//RBA_50_03_03_01
    ...
    function setData(gridData) {
    	...
    	//console.log("cnt["+cnt+"]");
    	var JIPYO_ALLT = 0;
    	var JIPYO_ALLT_INDV = 0;
    	var JIPYO_ALLT_CORP = 0;
    	var INDV_ID = "";
    	var CORP_ID = "";
    	...
    	for( i=0; i < cnt ; i++ ) {
    		...
    		JIPYO_ALLT_INDV = Number(cellData.RA_FIN_WEIGHT_INDV);
    		JIPYO_ALLT_CORP = Number(cellData.RA_FIN_WEIGHT_CORP);
    		
    		
    		INDV_ID = cellData.RSK_VALT_ITEM + "INDV";
    		CORP_ID = cellData.RSK_VALT_ITEM + "CORP";
    		...
    		if( i==0 ) {
    			form1.mltfFixyn.value = ( cellData.FIX_YN == "0") ? "N":"Y";
    		...
    		$('#mergeTable > tbody:last').append('<tr><td style="text-align:center;border-right: 1px solid #CCCCCC;">'+cellData.MODEL_NM+'</td>'
    											+'<td style="text-align:center;border-right: 1px solid #CCCCCC;" >'+cellData.RSK_CATG1_C_NM+'</td>'
    											...
    											+    '<input type="text" id="'+INDV_ID+'" style="text-align:center; width: 100%" value ="'+JIPYO_ALLT_INDV+'"/></td>'
    											+'<td style="text-align:center;border-right: 1px solid #CCCCCC;" >'
    											+    '<input type="text" id="'+CORP_ID+'" style="text-align:center; width: 100%" value ="'+JIPYO_ALLT_CORP+'"/></td>'
    											...
    											+'<td style="text-align:center;border-right: 1px solid #CCCCCC;" >'
    											+    '<input type="text" id="'+cellData.RSK_VALT_ITEM+'" style="text-align:center; width: 100%" value ="'+JIPYO_ALLT+'"/></td></tr>'
    	...
    	if(cnt > 0) {
    		...
    		$('#mergeTable > tbody:last').append('<tr  height="45px"><th style="text-align:center;border-right: 1px solid #CCCCCC;background-color:#D9E5FF; font-weight:bold" colSpan="7">SUM</th>'
    				                            +'<th style="text-align:center;border-right: 1px solid #CCCCCC;background-color:#D9E5FF; font-weight:bold" colSpan="1">RBA가중치(안):'+SUM_RBA_WEIGHT.toFixed(2)+'</th>'
    											+'<th style="text-align:center;border-right: 1px solid #CCCCCC;background-color:#B0C4DE; font-weight:bold" colSpan="1">최종가중치 합계 :'+SUM_RBA_FIN_WEIGHT.toFixed(2)+'</th></tr>');
    		form1.JIPYO_ALLT_SUM.value = SUM_RBA_FIN_WEIGHT;
    	}
    }
 ...
 // 분류가 같은 데이터 병합
    ...
    function doSave(confirm) {
    	...
    	/*최근 형가일정인지 체크  */
    	//if(!chkCommValidation("CHK_BAS_YYMM") && ROLE_ID != '7') {alert('${msgel.getMsg("RBA_50_02_01_010","최근 평가기준월 데이터만 등록,수정,삭제 가능합니다.")}'); return;};
    	/*업무 실제 종료일자 확인  */
    	//if(!doConfirmYn) {
    	//	if(!chkCommValidation("CHK_MODIFY_POSSI_REAL_DT") && ROLE_ID != '7') {alert('${msgel.getMsg("RBA_50_02_01_011","해당 업무단계의 실제종료일이 작성되어 있어 수정 불가능 합니다.")}'); return;};
    	//}

    	if(!mergeDataTot()) return;
    	if(form1.JIPYO_ALLT_SUM.value != "100")	 {alert("배점합계가 100점 만점이 되어야 합니다. 합계:["+form1.JIPYO_ALLT_SUM.value+"]"); return;};
    	...
    	for(var i=2; i<maxRow-1; i++) {
    		...
    		RSK_VALT_ITEM    = tabObj.rows[i].cells[rowLength-7].innerText;
    		RBA_FIN_WEIGHT   = $("#"+RSK_VALT_ITEM).val();
    		
    		INDV_ID       = RSK_VALT_ITEM + "INDV";
    		CORP_ID       = RSK_VALT_ITEM + "CORP";
    		
    		INDV_WEIGHT   = $("#"+INDV_ID).val();
    		CORP_WEIGHT   = $("#"+CORP_ID).val();
    		
    		
    		
    		
    		//JIPYO_ALLT       = $("#"+RSK_INDCT).val();

    		dataStr += RSK_VALT_ITEM+"#"+RBA_FIN_WEIGHT+"#"+INDV_WEIGHT+"#"+CORP_WEIGHT;
    		dataStr += ",";
    	}
    	dataStr = dataStr.substring(0,dataStr.length-1);


    	//alert( "Call onSave : " + dataStr);
		 ...
		 if(doConfirmYn) {
    		...
    		var methodID    = "doSave";
    		var obj = new Object();
    		obj.pageID      = pageID;					//"RBA_50_03_03_01";
    		obj.BAS_YYMM 	= $("#BAS_YYMM").val()		//평가년월
    		obj.dataArr 	= dataStr;
    		obj.doConfirmYn	= doConfirmYn ? "Y":"N";
	        ...
	        showConfirm("${msgel.getMsg('RBA_50_08_03_02_015','저장하시겠습니까?')}", "저장",function(){
	    		...
	    		var methodID    = "doSave";
	    		var obj = new Object();
	    		obj.pageID      = pageID;					//"RBA_50_04_02_01";
	    		obj.BAS_YYMM 	= $("#BAS_YYMM").val()		//평가년월
	    		obj.dataArr 	= dataStr;
	    		obj.doConfirmYn	= doConfirmYn ? "Y":"N";
	...
	function doRApopup() {
		...
		form2.pageID.value = "RBA_50_03_03_02";
        var win;       win = window_popup_open("RBA_50_03_03_02", 1300, 700, '','No');
        form2.target       = form2.pageID.value;
        form2.BAS_YYMM.value  = $("#BAS_YYMM").val();				//기준년월
        form2.action       = '<c:url value="/"/>0001.do';
        form2.submit();
	}
    ...
    /* function doConfirm(){
    	...
    	if(!mergeDataTot()) return;
    	if(parseInt(form1.JIPYO_ALLT_SUM.value) != 100)	 {alert("배점합계가 100점 만점이 되어야 합니다. 합계:["+form1.JIPYO_ALLT_SUM.value+"]"); return;};

    	var confirmState = "";
        if(form1.mltfFixyn.value == "N"){
            confirmState = "1";
            showConfirm("${msgel.getMsg('RBA_50_01_01_006','확정을 하시겠습니까?')}", "확정",function(){
                 var params   = new Object();
                 var methodID = "doConfirm";
                 ...
                 params.BAS_YYMM = form1.BAS_YYMM.value;
                 params.MLTF_FIX_YN = "1";  //confirmState
                 params.ING_STEP = "20";
                 params.RBA_VALT_LGDV_C = "C";
                 params.RBA_VALT_SMDV_C = "C02";
			...
			showConfirm("${msgel.getMsg('RBA_50_01_01_007','확정을 취소하시겠습니까?')}", "취소",function(){
				...
				params.pageID 	= pageID;
				params.BAS_YYMM = form1.BAS_YYMM.value;
				params.MLTF_FIX_YN = "0";  //confirmState
				params.ING_STEP = "10";
                params.RBA_VALT_LGDV_C = "C";
                params.RBA_VALT_SMDV_C = "C02";
    ...
    function doStartBatch(){
       ...
       if(form1.ING_STEP.value < 40) {
          showAlert('${msgel.getMsg("RBA_50_01_01_248","위험평가 가중치 확정이후에 배치수행 가능합니다.")}','WARN');
          return;
       }
      /*  if(!confirm("${BAS_YYMM}" + "의 통제평가 배치를 재수행 하시겠습니까 ?")) {
       ...
       showConfirm( $("#BAS_YYMM").val().substring(0,4) + "년 : "+ $("#BAS_YYMM").val().substring(5,6) + "회차의 고유위험 배치를 수행 하시겠습니까 ?","배치",function(){
    	   var params   = new Object();
  ...
  //배치수행
    function doStartRbaBatch(){

	  //talert( "shell : " + form1.Shell.value );         
              
       showConfirm( "/app/rba/AMLExpress7/RBA_Batch/dist/"+ form1.Shell.value + "   shell 수행하시겠습니까?","배치",function(){
    	   var params   = new Object();
    	   var methodID = "startRbaBatch";
    	   var classID  = "RBA_50_03_03_01";
    	   
    	   overlay.show(true, true);
    	    		
    	   params.pageID 	= pageID;
    	   params.Shell  = form1.Shell.value; //기준연도3
...
<form name="form1" method="post">
    ...
    <div class="inquiry-table type1" id='condBox1'>
        ...
        <div class="table-row" style="width:300px;">
        	<div class="table-cell">
            	${condel.getLabel("RBA_50_01_01_001","확정여부")}
            	<div class="content">
            		<input type="text" name= "mltfFixyn" size="2" class="cond-input-text" style="text-align:center" readonly="readonly" />
            	</div>
            </div>
        </div>
        <div class="table-row" style="width:300px;">
        ...
        <div class="table-row">	<!--  style="width:59.5%"     visibility: hidden;-->
        	<div class="table-cell">
        		<div class="content">
            		<input type="text" name= "Shell" size="30" class="cond-input-text" style="text-align:left;visibility: hidden;"  />
            		${btnel.getButton(outputAuth, '{btnID:"btn_30", cdID:"RBA022", defaultValue:"배치처리", mode:"C", function:"doStartRbaBatch", cssClass:"btn-36"}')}
            	</div>
            </div>
        </div>
    </div>
    ...
    <div class="button-area">
        ...
        ${btnel.getButton(outputAuth, '{btnID:"btn_02", cdID:"RBA_50_03_03_01", defaultValue:"RA가중치 가져오기", mode:"R", function:"doRApopup", cssClass:"btn-36"}')}
        ...
        <div class="panel-footer" >
            <div class="table-box" style="overflow-y:scroll;overflow-x:hidden;width:100%;height:72vh;table-layout:Fixed;" onscroll="scrollY()">
                <table width="100%" class="basic-table" id="mergeTable">
                    <tr height="35px" >
                        <th colspan="7" style="text-align:center;border-right: 1px solid #CCCCCC; background-color:#D9E5FF; font-weight:bold">${msgel.getMsg('RBA_50_03_03_01_001','RA')}</th>
                        <th colspan="2" style="text-align:center;border-right: 1px solid #CCCCCC; background-color:#B0C4DE; font-weight:bold">${msgel.getMsg('RBA_50_03_03_01_002','RBA')}</th>
                    </tr>
                    <tr height="35px" >
                        ...
                        <th style="text-align:center;border-right: 1px solid #B0C4DE; width:15%; background-color:#D9E5FF; font-weight:bold">${msgel.getMsg("RBA_50_03_03_01_010","RBA가중치(안)")}</th>
                        <th style="text-align:center;border-right: 1px solid #B0C4DE; width:15%; background-color:#B0C4DE; font-weight:bold">${msgel.getMsg("RBA_50_03_03_01_011","최종RBA가중치")}</th>
                    </tr>
                </table>
            </div>
        </div>
