...
//		 			},
    ...
    function doSearch() {
		...
		params.BRNO = $("#SEARCH_DEP_ID").val()=="99999"?"99999":$("#SEARCH_DEP_ID").val();
		...
		params.MOFC_BRN_CCD  = "B";				// 부점구분코드 ▶ 영업점
    ...
    function Grid1CellClick(id, obj, selectData, rowIdx, colIdx, colId) {
        ...
        if(colId == "IMPRV_S_C_NM"){
            ...
            form2.pageID.value     = "RBA_50_05_61_04";
...
//       				var classID  = "RBA_50_03_02_01";
					...
					var GYLJ_ROLE_ID; GYLJ_ROLE_ID = rowdata.GYLJ_ROLE_ID;
			 		var IMPRV_S_C   = rowdata.IMPRV_S_C;
					...
					//결재상태 확인
	       			...
	       			// 개선이행 상태가 "미조치" 이면 결재 불가
					if(rowdata.IMPRV_S_C === "N"){
			       		showAlert('${msgel.getMsg("RBA_50_05_05_01_204","개선이행 상태가 [미조치] 인 건은 결재를 진행할 수 없습니다.")}','WARN');
...
<form name="form1" method="post" >
    ...
    <input type="hidden" name="GYLJ_G_C"  value="RBA02">
...
<%--       	 		${JRBACondEL.getSRBASelect('','LV1' ,'P001' ,'160px' ,'' ,'' ,'ALL','doSearch()')} --%>
    ...
    <div class="button-area">
   		...
   		<% if ( "105".equals(ROLEID) || "7".equals(ROLEID)) { // 3:AML담당자,4:준법감시팀 담당자.5:준법감시팀 책임자,6:RBA/AML책임자,7:ADMIN,105:RBA 담당자,104:보고책임자 %>
    	...
    	<% } %>
   		<% if ( "6".equals(ROLEID) || "7".equals(ROLEID)) { // 3:AML담당자,4:준법감시팀 담당자.5:준법감시팀 책임자,6:RBA/AML책임자,7:ADMIN,105:RBA 담당자,104:보고책임자 %>
