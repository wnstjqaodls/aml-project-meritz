...
/*         if(form.GUBN.value == "0"){
    ...
    function doSearch(){
        ...
        GridObj1.refresh({
            actionParam: {
                "pageID"  : "RBA_50_08_06_02",
                "classID" : "RBA_50_08_06_02",
                "methodID": "getInAudtInfo",
                "isptDt"  : "${ISPT_DT}",
                "sNo"     : "${SNO}"
            },
    ...
    function doSave()
    {
        ...
        if ( getDxDateVal("ISPT_DT",true) == "" || getDxDateVal("ISPT_DT",true) == null ) {
        	alert("${msgel.getMsg('RBA_50_08_06_02_008','점검일자는 필수 입력항목입니다.')}");
        	return;
        }

        if ( frm.SEARCH_DEP_ID.value == "") {
        	alert("${msgel.getMsg('RBA_50_08_06_02_009','지점명은 필수 입력항목입니다.')}");
        	return;
        }
        
        if ( frm.INDI_CNT.value == "") {
        	alert("${msgel.getMsg('RBA_50_08_06_02_011','지적건수는 필수 입력항목입니다.')}");
        	return;
        }
        
        isNumChk = chkNum( frm.INDI_CNT.value );
        if ( isNumChk == "F") {
        	alert("${msgel.getMsg('RBA_50_08_06_02_012','지적건수항목에는 숫자를 입력해야 합니다.')}");
        	return;
        }
        
        if ( frm.MAIN_ISPT_CTNT.value == "") {
        	alert("${msgel.getMsg('RBA_50_08_06_02_010','주요점검내용은 필수 입력항목입니다.')}");
        	return;
        }
    ...
    function doUpdate()
    {
		...
		var obj = new Object();
        obj.pageID          = "RBA_50_08_06_02";
        obj.classID         = "RBA_50_08_06_02";
    ...
    function doRBASelectInputPopup60(pageId, width, height, searchName, afterFunction){
...
<div class="popup-cont-box" style="margin-top:1px;">
    <div class="table-box">
        <table>
        <tr>
            <th style="text-align:left;">${msgel.getMsg('RBA_50_08_06_01_001','감사일자')}*</th>
            <td>${condel.getInputDateDx('ISPT_DT', ISPT_DTB)}</td>
            <th style="text-align:left;">${msgel.getMsg('RBA_50_08_05_02_001','순번')}*</th>
            <td><input name="SNO" id="SNO" type="text" value="${SNO}" readonly></td>
        </tr>
        <tr>
            <th style="text-align:left;">${msgel.getMsg('RBA_50_08_06_01_003','지점명')}*</th>
            <td>
            	<RBATag:searchRbaInput searchName="SEARCH_DEP_ID" searchClass="cond-select" searchStyle="width: 60px;" text1Name="DEP_ID" 
					...
					popupFunction='doRBASelectInputPopup60("searchRbaDepPopup", 480, 580, "SEARCH_DEP_ID", "setRBASearchInputPopup(searchName, searchInfo)")' 
					searchValue="" text1Value="" text2Value=""/>
            </td>
            <th style="text-align:left;">${msgel.getMsg('RBA_50_08_06_01_002','개선구분')}*</th>
            <td>
...
<div class="popup-cont-box">
    <div class="cond-btn-row" style="text-align:right;margin-top:10px;">
        ...
        <tr>
            <td><font color=blue> - 02O2080101 (2.8.1) 평가일 기준 최근1년간 평가대상 부서 대비 AML 관련 독립적 감사 점검을 수행한 부서 비율</font></td>
        </tr>
        <tr>
            <td><font color=blue> - 02O2080101, 02O2080301 (2.8.2) 평가일 기준 최근1년간 AML 관련 감사 지적사항에 대한 개선 비율</font></td>
        </tr>
        <!-- 유안타증권 - BSL END -->
        </table>
</div>
