...
<script>
    ...
    $(document).ready(function(){
    	$('.popup-header > h2').text("${msgel.getMsg('adm.comm.popup.user.title','사용자 검색')}");
//         setPageTitle("${msgel.getMsg('adm.comm.popup.user.title','사용자 검색')}",true);
    ...
    function closeOK()
    {
        ...
        if (selectedItem && selectedItem.USER_ID && opener['<c:out value="${param.FUNC_NM}"/>']) {
        	if($.isFunction(opener['<c:out value="${param.FUNC_NM}"/>'])) {
            	var funcRet = opener['<c:out value="${param.FUNC_NM}"/>']({'LOGIN_ID':selectedItem.LOGIN_ID,'USER_NM':selectedItem.USER_NM});
    ...
    function userSearch()
    {
    	...
    	gridUser.option('dataSource', []);
    	
        var params = new Object();
        params.actionName = "com.gtone.express.server.actions.query.MQueryAction";
    	params.DS_TYPE = "JSPEED";
    	params.LANG_CD = '${LANG_CD}';
    	params.CALL_TYPE = 'server.actions.query.ExecuteQueryAction.commUserGridData';
    	params.USER_NM = $("input[name=txtUSER_NM]").val();
    	sendService("1", "1", params, userSearch_success, doSearch_fail);
    }
