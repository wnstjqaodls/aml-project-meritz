 ...
 */
package com.gtone.rba.server.type03.RBA_50.RBA_50_01.RBA_50_01_08;
 ...
 */
public class RBA_50_01_08_01 extends GetResultObject {

	private static RBA_50_01_08_01 instance = null;
	/**
	* getInstance
	* @return RBA_50_01_08_01
	*/
	public static  RBA_50_01_08_01 getInstance() {
		synchronized(RBA_50_01_08_01.class) {  
		    if (instance == null) {
			    instance = new RBA_50_01_08_01();
		    }
		}
		return instance;
	}
	...
	*/
	public DataObj doSearch(DataObj input) {
		...
		try {
			output = MDaoUtilSingle.getData("RBA_50_01_08_01_getSearch", input);
			// grid data
			if (output.getCount("GYLJ_LINE_G_C") > 0) {
	...
	*/
	public DataObj doSearch2(DataObj input) {
		...
		try {
			output = MDaoUtilSingle.getData("RBA_50_01_08_01_getSearch2", input);
			// grid data
			if (output.getCount("GYLJ_LINE_G_C") > 0) {
				gdRes = Common.setGridData(output);
			}
    ...
    public DataObj doSave(DataObj input) throws AMLException {
		...
		try {
			...
			for (int i = 0; i < gdReq_size; i++) {
				...
				mDao.setData("RBA_50_01_08_01_mergeSRBA_GRP_C", inputMap);	
			}
    ...
    public DataObj doSave2(DataObj input) throws AMLException {
		...
		try {
			...
			for (int i = 0; i < gdReq_size; i++) {
				...
				mDao.setData("RBA_50_01_08_01_mergeSRBA_DTL_C", inputMap);	
			}
	...
	*/
    ...
    public DataObj doDelete(DataObj input) throws AMLException {
		...
		try {
			...
			// 상세코드가 있는경우 삭제금지!
			int cnt = 0 ;
			for (int k = 0; k < gdReq_size; k++) {
				HashMap inputMap_CNT = (HashMap) gdReq.get(k);					
				output = MDaoUtilSingle.getData("RBA_50_01_08_01_getCount", inputMap_CNT);
				if (output.getInt("COUNT") != 0 ){
					cnt = cnt + output.getInt("COUNT") ;
				}					
			} 
			...
			if (cnt == 0){				
				...
				for (int i = 0; i < gdReq_size; i++) {
					HashMap inputMap = (HashMap) gdReq.get(i);
					mDao.setData("RBA_50_01_08_01_Delete", inputMap);
				}
				...
				output.put("ERRCODE", "00000");
				output.put("ERRMSG", "결재선상세가 있는 경우 삭제 불가능합니다.");
				output.put("WINMSG", "결재선상세가 있는 경우 삭제 불가능합니다.");
			}
	...
	*/
    ...
    public DataObj doDelete2(DataObj input) throws AMLException {
		...
		try {
			...
			for (int i = 0; i < gdReq_size; i++) {
				HashMap inputMap = (HashMap) gdReq.get(i);
				mDao.setData("RBA_50_01_08_01_Delete2", inputMap);
			}
