...
<% 
	String dis_flag    = Util.nvl(request.getParameter("dis_flag"));
	String EVLTN_STATE = Util.nvl(request.getParameter("EVLTN_STATE"));
	String APP_NO      = Util.nvl(request.getParameter("APP_NO"));
	String PRD_EVLTN_ID      = Util.nvl(request.getParameter("PRD_EVLTN_ID"));

	request.setAttribute("dis_flag"    , dis_flag   );
	request.setAttribute("EVLTN_STATE" , EVLTN_STATE   );
	request.setAttribute("APP_NO"      , APP_NO   );
	request.setAttribute("PRD_EVLTN_ID"      , PRD_EVLTN_ID   );
%>
<script language="JavaScript">
	var dis_flag    = "${dis_flag}";
	var EVLTN_STATE = "${EVLTN_STATE}";
	var APP_NO 		= "${APP_NO}";
	var PRD_EVLTN_ID = "${PRD_EVLTN_ID}";
	
	function doSave(){  
	
	   if("DS" == dis_flag) {
		   parent.window.opener.focus();		
		   parent.window.opener.openerView.doAppr( $("#RSN_CNTNT").val(), "DS", EVLTN_STATE, APP_NO, PRD_EVLTN_ID);
		   window.close();
	   }else if("DD" == dis_flag) {
		   parent.window.opener.focus();		
		   parent.window.opener.openerView.doAppr( $("#RSN_CNTNT").val(), "DD", "DD", APP_NO, PRD_EVLTN_ID);
		   window.close();
	   }else {
		   parent.window.opener.focus();		
