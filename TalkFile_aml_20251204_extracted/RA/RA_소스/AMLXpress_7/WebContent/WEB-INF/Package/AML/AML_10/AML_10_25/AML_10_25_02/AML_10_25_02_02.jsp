...
<script language="JavaScript"> 
    ...
    function doSearch_end(gridData, actionParam){    	
	    ...
	    sendService("AML_10_25_02_01", "getCodeSearch", params, doCdSearch_End, doCdSearch_End); 
     	...
     	if(ROLEID == "107") { 
     		if(EVLTN_STATE == "S1" || EVLTN_STATE == "S2" || EVLTN_STATE == "S3" || EVLTN_STATE == "R" || EVLTN_STATE == "E") {
     			$("#btn_01").hide();
     	     	$("#btn_02").hide();		
            }else {
        	...
        	if(EVLTN_STATE == "S1" || EVLTN_STATE == "S3" || EVLTN_STATE == "R" || EVLTN_STATE == "E" || EVLTN_STATE == "N") {
        		$("#btn_03").hide();
     	     	$("#btn_04").hide();
        	}else {
        		$("#btn_03").show();
    ...
    function CheckValue(){
	    ...
	    if( "4" != ROLEID && (rlsExpctDt < todayDate) ){
        ...
        if( "4" != ROLEID && (evltnPrfDt < todayDate) ){
		...
		if( !isCkAbled2 && isEmpty($("#SMRY_OPNN_TMP").val()) ){
   ...
   function doPrdApprReq(){
	  ...
	  showConfirm('${msgel.getMsg("AML_10_25_01_02_024","승인요청 하시겠습니까?")}','승인요청', function(){
		  ...
		  form2.dis_flag.value = "P";
		  form2.pageID.value = 'AML_10_25_02_04';
		  form2.target = 'AML_10_25_02_04';
		  form2.action = '${path}/0001.do';
		  form2.method = "post";
   ...
   function doAppr( rsnCntnt, target){
	  ...
	  if( target == 'R' ){
		  ...
		  if( "107" == ROLEID ){
	    	 ...
	    	 form1.SN_CCD.value = 'S3'; 
	    	 form1.EVLTN_STATE.value = 'S3';
	    	 form1.FIRST_SNO.value = '3'
	      }
      }
  ...
  <br/>
    ...
    <div class="tab-content-bottom">
	 ...
	 <div class="button-area" style="display:flex; justify-content: flex-end; padding-top: 8px; padding: 14px 20px 10px 0;">
	   ...
	   <% if( "107".equals(ROLEID) && !"SN".equals(EVLTN_STATE) ) { // 신상품 담당자  %>
		 <button type="button" onclick="doSave()" id="btn_01" class="btn-36" mode="U">${msgel.getMsg("AML_10_20_01_01_012","저장")}</button>
		 <button type="button" onclick="doPrdApprReq()" id="btn_02" class="btn-36" mode="U">${msgel.getMsg("AML_10_25_01_02_018","승인요청")}</button> 
	   <% } %>
	   
	   <% if( "4".equals(ROLEID) ) { // 자금세탁사기예방팀 담당자 %>
		 <button type="button" onclick="doPrdAppr()" id="btn_03" class="btn-36" mode="U">${msgel.getMsg("AML_10_25_01_02_019","결재")}</button>
